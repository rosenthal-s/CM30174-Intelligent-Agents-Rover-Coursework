...
+!start:	true
	<-	.print("hello world.");
		...
		+capacity( Capacity ); /// 6 is hard-coded because I don't know how to check if two beliefs have the same value
...
-!return_to_resource:	finished( 1 ) ///
	<-	.send( jeweller, achieve, new_move ); /// put elsewhere?
		.

+!collect_resource: true
	<-	?resource_type( ResourceType );
		.print( "Collecting up to 6 ", ResourceType, "!" )
		!collect_recursive;
	    .
